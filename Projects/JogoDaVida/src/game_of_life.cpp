...
void GameOfLife::testException(int i, int j, int M, int N) {
  if(0 > i || i >= M || 0 > j || j >= N) {
    GameOfLife::InvalidCellException newExp{i, j};
    throw newExp;
  }
}
...
void GameOfLife::Enliven(int i, int j) {
  this->testException(i, j, this->lines(), this->columns());
  torus_[i][j] = true;
}
...
bool GameOfLife::alive(int i, int j) const {
  GameOfLife::testException(i, j, lines(), columns());
  return torus_[i][j];
}
